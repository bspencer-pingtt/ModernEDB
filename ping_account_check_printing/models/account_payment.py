...
class AccountPayment(models.Model):
    ...
    def action_post(self):
        ...
        payment_method_check = self.env.ref('ping_account_check_printing.account_payment_method_check')
    ...
    def do_print_checks(self):
        check_layout = self.company_id.ping_account_check_printing_layout
    ...
    def _check_build_page_info(self, i, p):
        multi_stub = self.company_id.ping_account_check_printing_multi_stub
        return {
            'sequence_number': self.check_number,
            'manual_sequencing': self.journal_id.check_manual_sequencing,
            'date': format_date(self.env, self.date),
            'year': self.date.year,
            'year1': list(str(self.date.year))[0],
            'year2': list(str(self.date.year))[1],
            'year3': list(str(self.date.year))[2],
            'year4': list(str(self.date.year))[3],
            'month': self.date.month,
            'month1': (list(str(self.date.month))[0]) if len(str(self.date.month)) > 1 else str(0),
            'month2': (list(str(self.date.month))[1]) if len(str(self.date.month)) > 1 else list(str(self.date.month))[0],
            'day': self.date.day,
            'day1': (list(str(self.date.day))[0]) if len(str(self.date.day)) > 1 else str(0),
            'day2':(list(str(self.date.day))[1]) if len(str(self.date.day)) > 1 else list(str(self.date.day))[0],
    ...
    def _check_make_stub_pages(self):
        ...
        if not self.company_id.ping_account_check_printing_multi_stub:
